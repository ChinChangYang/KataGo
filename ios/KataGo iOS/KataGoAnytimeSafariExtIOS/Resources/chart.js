...
class KGAChart {
    ...
    constructor(canvas, callbacks) {
        ...
        // iOS blends winrate only: the bundled tiny net has no reliable score
        // head, so the score pane is suppressed and winrate takes the height.
        this.showScore = this.callbacks.showScore !== false;
    ...
    layout() {
        ...
        const gap = this.showScore ? 10 : 0;
        const winH = this.showScore ? Math.round((h - padT - padB - gap) * 0.62)
                                    : (h - padT - padB);
    ...
    draw() {
        ...
        if (this.showScore) { this.drawScore(ctx, l); }
