...
* **Item 15** is a data-loss regression check. It guards the build-291 class of
...
* **Item 25** is a question nothing in the code answers yet: does an extended
  ...
  guard against this today, so item 25 decides whether one is needed.
...
4. Sit ~10 feet from the TV for the legibility items (8 and 41).
...
1. **Start/stop.** Open Game U. Press **Play/Pause**: stones start appearing
   ...
   **play** glyph. *(Verified in Simulator — confirm on the real remote.)*
   ...
   exception is the board in aiming mode, item 29). This is a responder-chain
   assumption that has never been checked from more than one focus location.
2. **Speed.** Settings ▸ Playback ▸ Auto-Play Speed ▸ **Fast**, go back into
...
3. **Timeline click stops it.** While replaying, click the timeline left or
...
4. **Board focus stops it.** While replaying, move focus left onto the board:
   Auto-Play stops and the ghost cursor appears on the board.
5. **Top Moves stops it.** While replaying, pick a Top Move: Auto-Play stops
...
6. **Analysis is never touched.** Replay with Analysis **On**, then with
   ...
   numbers.
7. **No screensaver.** Replay a long game to the end without touching the
   remote: the tvOS screensaver never covers the board mid-replay.
8. **Layout at 10 feet.** Both controls in the bottom row (the icon-only
...
9. **No dead press.** With Auto-Play running, and again with a variation active,
   press **down** from the timeline: focus always lands on something legal —
   never a press that does nothing.
10. **Focus between the two pills.** Move focus left and right between the
...
11. **Disabled pill must not strand focus.** Put focus **on the Auto-Play pill**,
...
12. **Leaving the screen stops it.** With Auto-Play running, press Menu (or
    switch to another tab): the replay stops, and returning to the game does not
    resume it by itself.

## B. Handoff to a live continuation

13. **The handoff.** Replay Game U to its **last** recorded move:
...
14. **The return.** When that continuation ends, the result card reads
    "Returning to review…" and the app pops back to the review screen — not to
    the library.
15. **⚠️ THE REGRESSION CHECK.** After that pop: play a **Top Moves** move on
...
16. **Analysis state survives the round trip.** Note whether Analysis was on or
    off before the handoff; after the pop back to review it must be in the
    **same** state, with the eye icon and the panel agreeing.
17. **No flash on the pop.** Watch the moment of the pop: the review screen must
    come back showing the recorded position it left from — no empty board, no
    board that briefly shows the continuation's stones.
18. **A finished game never hands off.** Replay Game F to its last move: it
    stops on the final position with **no** "Continuing live…" overlay and no
    push.
19. **Manual stepping never hands off.** With the D-pad, step Game U manually to
    its last recorded move: nothing is pushed, no overlay.
20. **Parked at the end.** Park on Game U's last move and press Play/Pause: the
    handoff starts immediately (overlay, then push).
21. **From Search.** Repeat item 13 for Game U opened from the **Search** tab.
22. **Cancel the beat.** Start the handoff and press **Menu** during the
...
23. **Hammer the beat.** Start the handoff and, during the 2 s beat, press
    ...
    re-run item 15's check on the recorded game.

## C. Game controller

24. **Detection.** With the pad paired, Settings shows a **Game Controller**
...
25. **⚠️ Does X double-fire?** On the review screen, with Auto-Play **stopped**,
...
26. **Navigation parity.** The D-pad moves focus, **A** selects, **B** goes
    back — exactly as the Siri Remote does, on the library, the review screen
    and Settings.
27. **Review bindings.** On the review screen: **X** toggles Auto-Play (pill
...
28. **L1 at move 0.** Press and hold **L1** while already at move 0: nothing
    happens at all — no flicker, no re-analysis.
29. **Aiming makes them inert.** Focus the board (aiming/ghost-cursor mode):
...
30. **Live-screen bindings.** On the live broadcast: **X** pauses and resumes,
    **R1** skips the current slide, **L1** undoes **while paused**.
31. **L1 held on the live screen.** Hold **L1** on the paused live screen. The
...
32. **Attract mode ignores the mapping.** Leave the app idle until the
...
36. **No stray handler.** Press X, Y, L1, R1, L2 and R2 on the **library** and
    **Settings** screens: nothing happens (no hidden Auto-Play on a screen you
    ...
    tab: still nothing.
37. **Two pads.** Pair a second controller. Settings must name the pad you are
    actually pressing, and pressing X on either pad must drive the screen you
    are on.
38. **Siri Remote after the pad.** Use the Siri Remote for a few presses (this
...
39. **System button remapping.** In tvOS Settings ▸ Remotes and Devices ▸ your
...
40. **VoiceOver.** Turn VoiceOver on and focus the icon-only Auto-Play pill: it
...
41. **Legibility of the Settings legend.** At 10 feet, the Game Controller
...
* **Any gamepad button press.** A real DualSense is paired to the Mac and the
  ...
  items 25-39 are entirely unexercised — button bindings, the LIFO handler
...
* All of section D, and items 8, 10, 11 (layout, focus and VoiceOver on a real
