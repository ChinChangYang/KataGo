...
public enum SampleGames {
    /// The complete 325-move record. Keep RU[Japanese]: `loadGame` re-derives
    /// the rules from the SGF through the bridge's `SgfCpp::getRules`, which
    /// catches `getRulesOrFail` into an ALL-DEFAULT rule set with komi 7.0 when
    /// there is no rules tag — so without it this game would replay under
    /// Chinese-ish defaults and lose its `KM[0]`. It is NOT a crash: the
    /// engine's `loadsgf` uses `getRulesOrWarn` (falls back to the current
    /// rules and logs) inside a try/catch, verified by reading
    /// `cpp/command/gtp.cpp` and by importing a tag-less SGF.
    /// KM[0]: the historical game had no komi.
