...
| 297 | `…config.getKataKomiCommand())` | `ConfigEngineSync.setKomi(Float(newValue) ?? Config.defaultKomi, config: config, messageList: messageList)` — pass the RAW parsed value (not stale `config.komi`); keep `isRuleChanged = true` |
| 386 | `…config.getKataAnalysisWideRootNoiseCommand())` | `ConfigEngineSync.setAnalysisWideRootNoise(<raw parsed new value the inline code used>, config: config, messageList: messageList)` — pass the RAW value, not stale `config.analysisWideRootNoise` |
...
> CRITICAL — pass the RAW new value, preserve surrounding local lines. Each `onChange` computes the property from the raw input (e.g. komi: `config.komi = min(1_000, max(-1_000, ((Float(newValue) ?? Config.defaultKomi) * 2).rounded() / 2))`) then sends. `ConfigEngineSync.set*` applies the IDENTICAL clamp+set internally, so pass it the RAW value the inline code parsed (e.g. `Float(newValue) ?? Config.defaultKomi`), NOT `config.X` (stale once the inline write is removed). Replace ONLY the inline `config.X = …` write and the `messageList.appendAndSend(command: config.getX())` send with the single `ConfigEngineSync.set*` call; PRESERVE every other line in the closure verbatim (e.g. `isRuleChanged = true`, `blackHumanSLModel.profile = newValue` / `whiteHumanSLModel.profile = newValue`). Before replacing each site, READ the closure and confirm the chosen `ConfigEngineSync` method reproduces its exact effect (same stored-property write + same command/commands). If it would NOT (different stored property, different commands source), leave that site inline and report it.

> LEAVE the prop-only sites INLINE — do NOT migrate them: analysisForWhom 360–362, maxAnalysisMoves 393–395, analysisInterval 401–403, blackMaxTime 465–467, whiteMaxTime 498–500. On iOS these set a config property with NO immediate GTP (analysis re-arms downstream). The matching `ConfigEngineSync` setters (`setMaxAnalysisMoves`/`setAnalysisInterval`/`setBlackMaxTime`/`setWhiteMaxTime`) call `rearmAnalysis`, so routing iOS through them would ADD an immediate re-arm iOS does not currently do — a behavior change. Behavior preservation wins: keep these `config.X = newValue` writes exactly as they are. They call no `config.getX()` generator, so they do not block B5's generator deletion.
