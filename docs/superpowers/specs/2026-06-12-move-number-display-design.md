...
4. **Marker** — a small triangle (classic kifu convention) on the last move,
   no number.
...
- **Marker** — small triangle outline on the last move's stone, contrasting
  color, roughly one third of the square size.
...
- Marker style is a **triangle** (user choice; classic kifu convention).
