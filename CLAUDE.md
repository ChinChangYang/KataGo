...
- `KataGo Anytime Vision/` - visionOS app target (volumetric RealityKit 3D goban; scheme `KataGo Anytime Vision`, product "KataGo Vision"): board input is game-controller-only (thumbstick/D-pad ghost stone, A play, B dismiss, L1/R1 candidate cycle, X undo, Y pass — immediate, no confirmation; Undo is the safety net); a bottom-front ornament carries player chips (pinch to flip Human⇄AI via `ConfigEngineSync.set*MaxTime`), New Game (9/13/19), Pass, Undo, the analysis eye, and a controller-mapping legend (auto-shown on first controller connect). The root view owns the continuous-analysis re-arm on the `waitingForAnalysis` true→false edge (mirrors GameSplitView.processChange — without it analysis freezes after the first move). Only square 9/13/19 games render (bundled `Resources/BoardAssets/` USDZs + placement manifest; other sizes gate to an unsupported-board state). Engine pinned CoreML/ANE-only `[100, 100]` with NN buffer cap 19. Platform-agnostic logic (BoardAssetManifest, BoardSceneGeometry, GhostCursorModel, visionBoardIsSupported) lives in `KataGoUICore/Sources/KataGoUICore/Vision/` so the iOS-simulator test target covers it.
