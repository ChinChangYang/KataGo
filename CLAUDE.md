...
- `KataGo Anytime Vision/` - visionOS app target (volumetric RealityKit 3D goban; scheme `KataGo Anytime Vision`, product "KataGo Vision"): board input is game-controller-only (thumbstick/D-pad ghost stone, A play, B show/hide analysis, X/L1 one move back and R1 one move forward (L1/R1 hold to auto-repeat; the played/undone stone flies on/off the board via `StoneAnimationPlanner`), L2/R2 jump to start/end of game, Y pass — immediate, no confirmation; Undo is the safety net); a bottom-front ornament carries player chips (pinch to flip Human⇄AI via `ConfigEngineSync.set*MaxTime`), a Games toggle (shows/hides a left-side game-list ornament of the newest iCloud-synced games, whose header carries New Game — 9/13/19 quick sizes disabled above the engine cap, plus a Custom… card with width/height steppers 2..cap, any rectangle; picking runs the same gated `switchGame` path as boot), the analysis sparkle (run/pause/off), a Settings gear (right-side card: Analysis information picker Winrate/Score/All/None — Vision default Winrate; Show ownership toggle; board orientation tabletop ⇄ standing — persisted via `VisionSettings.*` UserDefaults on `VisionGameShell`, mirrored into `GobanState`; a Max Board Size picker 9/13/19/37 — default 19, persisted per model — that quits and respawns the engine with the new NN buffer **in the background, with the goban still on screen** — `VisionEngineController` owns the restart, with read-loop parking in `VisionRootView`), and a controller-mapping legend (auto-shown on first controller connect; Settings, the legend, and the New Game card share the right anchor, mutually exclusive). The root view owns the per-move engine driver: `.onChange(of: player.nextColorForPlayCommand)` calls the shared `GobanState.handleTurnChange` (the same body `BoardView` runs: asymmetric human-SL commands → `maybeRequestAnalysis` → clear) — without it the AI never auto-replies and analysis freezes after the first move. **Any board from 2×2 to 37×37 renders, square or rectangular**: the bundled USDZs are geometry-only squares (a rectangle reuses the width-matched square with its slab depth-stretched and legs repositioned), the board-top texture (procedural wood + grid + hoshi from the shared `BoardStarPoints` rule) is generated at load by `BoardTopTexture` and swapped over the 4×4 placeholder before mount, and placement is analytic (`BoardGeometryRules` — no manifest). A board over the launched NN buffer still renders — it is a *Held* engine status (`EngineHeldRule`): the engine is told nothing about that board, the ornament's sparkle wears a warning badge, and its tap opens the Models card whose status header points at the remedy (raise Max Board Size, or switch nets when the net itself is capped); only geometry outside 2..37 is a true gate (unsupported). The volume is 0.9×0.95×0.9 m (`VisionVolumeMetrics`, 1224×1292×1224 pt at 1360 pt/m) so a 37×37 fits both orientations at the native 22×23.7 mm pitch. Engine pinned CoreML/ANE-only `[100, 100]`; NN buffer defaults to 19. Platform-agnostic logic (BoardGeometryRules, BoardTopTexture, BoardSceneGeometry, GhostCursorModel, StoneAnimationPlanner, visionBoardIsSupported, VisionGamePickerItem) lives in `KataGoUICore/Sources/KataGoUICore/Vision/` so the iOS-simulator test target covers it.
...
| `ContentView.swift` | iOS board tree; mounts on the first frame, seeds the initial game engine-free, and runs the read loop keyed on `AppEngineController.readLoopGeneration` |
| `AppEngineController.swift` | iOS engine lifecycle: launch, restart (model / backend / Max Board Size / Retry), thread-exit classification, Held |
...
| `GameSession.swift` | Per-game engine driver; owns the GTP message loop (`messaging()`), the bounded `handshake()`, and `engineStatus` |
| `RecordPosition.swift` | `RecordPositionProjector` — the only writer of the board's stones, replayed from the record's SGF; `RecordPositionKey` identifies the displayed position |
| `EngineFeed.swift` | Pure builder for the move-by-move GTP feed (opening bundle, forward `play`s, `undo` counts, setup command) |
| `EngineStatus.swift` | `EngineAvailability` (Absent / Launching / Ready / Failed / Held), its strings (`EngineStatusText`), `EngineHeldRule`, `EngineExitDisposition` |
...
**Locations:** `KataGo_iOSApp.swift`, `ContentView.swift`, `AppEngineController.swift`, `GameSplitView.swift`, and `GobanView.swift` are in the iOS app target (`KataGo iOS/`); `MainWindowController.swift` and `BoardViewController.swift` are in the macOS target (`KataGo Anytime Mac/`, alongside `SubprocessKataGoEngine.swift`); the `Vision*.swift` files are in the visionOS target (`KataGo Anytime Vision/`). The rest live in the shared `KataGoUICore` package: Bridge (`KataGoHelper.swift`, `KataGoEngineIO.swift`), Session (`GameSession.swift`, `RecordPosition.swift`, `EngineFeed.swift`), Model (`KataGoModel.swift`, `GameRecord.swift`, `GobanState.swift`, `NeuralNetworkModel.swift`), Services (`Commentator.swift`, `AudioModel.swift`, `EngineStatus.swift`, `EngineRestartRules.swift`), Rendering (`LinePlotView.swift`, `BoardLineView.swift`, `EngineStatusView.swift`).
...
**The board never waits for this.** Stones come from the game record, not from the engine (ADR 0008): `RecordPositionProjector` replays the record's SGF and is the only writer of the board's stones, so every platform mounts its board on the first frame and shows engine availability as an inline `EngineStatusView` line instead of a loading screen. Commands are gated on a **handshake**: `GameSession.handshake()` sends `version` (a *lifecycle* command — `version`/`stop`/`quit` bypass the gate) and reads with a bounded, cancellation-aware timed read until a `= `/`? ` reply, EOF or the 660 s budget. Until it succeeds `MessageList.isAcceptingCommands` is `false` and `appendAndSend` **drops and logs** every other command; the feed that was dropped is recorded on `GobanState.engineSyncGate` and replayed against the *live* selection once the engine answers.
...
- `showboard` - The **sync acknowledgement**: its `= MoveNum` reply is what marks the engine in sync with the record position (it no longer supplies the board's stones)
- `printsgf` - Export game as SGF
- `clear_board` - Starts every open/switch, before the setup stones
- `set_free_handicap <vertices>` - Setup stones when every one is Black (leaves White to move, applies the handicap bonus); requires an empty board
- `set_position <color> <vertex> …` - Setup stones for any other AB/AW combination (leaves Black to move)
- `play <color> <move>` - Make moves; also how the engine is fed, one recorded move at a time
- `undo` - Backward navigation, refusal-aware (a move the replay refused was never played, so it is never undone)
- `kata-set-rule` - Configure rules

**`loadsgf` is no longer used by the apps** (ADR 0008): a game is fed move by move so that a single illegal move cannot discard the whole record. The two Safari extension appexes keep their own `loadsgf`-based flow.
